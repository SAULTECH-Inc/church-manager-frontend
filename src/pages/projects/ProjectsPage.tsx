  ...
  return (
    <div style={{ padding: '24px 32px', minHeight: '100vh', backgroundColor: PAGE_BG }}>
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 style={{ color: 'white', fontWeight: 700, fontSize: 26, margin: 0 }}>Project Management</h1>
          <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 14, marginTop: 4 }}>Building · Mission · Charity · Outreach</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 16, marginBottom: 24 }}>
        {[
          { label: 'Total Projects', value: stats?.total ?? 0, color: '#7c6bff' },
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' as const }}>
        {['ALL', 'PLANNING', 'ACTIVE', 'ON_HOLD', 'COMPLETED', 'CANCELLED'].map(s => (
          <button key={s} onClick={() => setFilter(s)} style={{
            ...
            border: `1px solid ${filter === s ? ACCENT : 'rgba(255,255,255,0.1)'}`,
            backgroundColor: filter === s ? 'rgba(124,107,255,0.15)' : 'rgba(255,255,255,0.04)',
            color: filter === s ? ACCENT : 'rgba(255,255,255,0.5)',
          }}>{s.replace('_', ' ')}</button>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 16 }}>
        {filtered.length === 0 && !isLoading && (
          <div style={{ backgroundColor: CARD, borderRadius: 20, border: '1px solid rgba(255,255,255,0.08)', padding: 40, textAlign: 'center' }}>
            <p style={{ color: 'rgba(255,255,255,0.4)' }}>No projects yet. Create your first project.</p>
          </div>
        )}
        {filtered.map(p => {
          ...
          return (
            <div key={p.id} style={{ backgroundColor: CARD, borderRadius: 20, border: '1px solid rgba(255,255,255,0.08)', overflow: 'hidden' }}>
              {/* Project header */}
              <div style={{ padding: '18px 24px' }}>
                <div className="flex items-start justify-between gap-4">
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div className="flex flex-wrap gap-4 mt-2">
                      ...
                      {p.startDate     && <span style={{ color: 'rgba(255,255,255,0.45)', fontSize: 12 }}>📅 {p.startDate}{p.endDate ? ` → ${p.endDate}` : ''}</span>}
                    </div>
                  </div>
                ...
                {milestones.length > 0 && (
                  <div style={{ marginTop: 12 }}>
                    ...
                    <div style={{ height: 4, borderRadius: 4, backgroundColor: 'rgba(255,255,255,0.08)' }}>
              ...
              {isOpen && (
                <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', padding: '18px 24px' }}>
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-3">
                        <h4 style={{ color: 'white', fontWeight: 700, fontSize: 13, margin: 0 }}>Milestones</h4>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 8 }}>
                        {milestones.length === 0 && <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: 12 }}>No milestones yet.</p>}
                        {milestones.map(m => (
                          <div key={m.id} className="flex items-start gap-2">
                            <button onClick={() => toggleMs.mutate(m.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, marginTop: 1, flexShrink: 0 }}>
                              {m.completed ? <CheckCircle size={16} color="#10b981" /> : <Circle size={16} color="rgba(255,255,255,0.3)" />}
                            </button>
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <p style={{ color: m.completed ? 'rgba(255,255,255,0.4)' : 'white', fontSize: 13, margin: 0, textDecoration: m.completed ? 'line-through' : 'none' }}>{m.title}</p>
                              {m.dueDate && <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: 11, margin: '2px 0 0' }}>Due: {m.dueDate}</p>}
                            </div>
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-3">
                        <h4 style={{ color: 'white', fontWeight: 700, fontSize: 13, margin: 0 }}>Donations</h4>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 6 }}>
                        {donations.length === 0 && <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: 12 }}>No donations recorded.</p>}
                        {donations.map(d => (
                          <div key={d.id} className="flex items-center justify-between">
                            <div>
                              <p style={{ color: 'white', fontSize: 13, margin: 0 }}>{d.donorName ?? 'Anonymous'}</p>
                              <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: 11, margin: 0 }}>{d.donationDate}</p>
                            </div>
                  ...
                  {(p.contractorName || p.contractorContact) && (
                    <div style={{ marginTop: 16, padding: '12px 16px', backgroundColor: 'rgba(255,255,255,0.03)', borderRadius: 12, border: '1px solid rgba(255,255,255,0.06)' }}>
                      <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 4 }}>Contractor</p>
                      <p style={{ color: 'white', fontSize: 13, margin: 0 }}>{p.contractorName}</p>
                      {p.contractorContact && <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 12, margin: '2px 0 0' }}>{p.contractorContact}</p>}
                    </div>
                  )}
                </div>
              )}
