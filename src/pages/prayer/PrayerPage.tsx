...
const prayerStatusColor = (s: string) => {
  ...
  if (s === 'IN_PRAYER') return { color: 'var(--accent)', bg: 'rgba(124,107,255,0.15)' }
  if (s === 'ANSWERED') return { color: '#34d399', bg: 'rgba(52,211,153,0.15)' }
  return { color: 'rgb(var(--inv) / 0.4)', bg: 'rgb(var(--inv) / 0.06)' }
}
...
function Toggle({ value, onChange, label }: { value: boolean; onChange: (v: boolean) => void; label: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', backgroundColor: 'rgb(var(--inv) / 0.04)', borderRadius: 12, padding: '12px 16px' }}>
      ...
      <button onClick={() => onChange(!value)} style={{ width: 44, height: 24, borderRadius: 12, border: 'none', cursor: 'pointer', backgroundColor: value ? '#7c6bff' : 'rgb(var(--inv) / 0.1)', position: 'relative' }}>
  ...
  return (
    <div style={{ padding: '24px 32px', minHeight: '100vh', backgroundColor: 'var(--page-bg)' }}>
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: 26, margin: 0 }}>Prayer & Testimonies</h1>
          <p style={{ color: 'rgb(var(--inv) / 0.5)', fontSize: 14, marginTop: 4 }}>Prayer Requests · Testimonies · Intercession</p>
        </div>
      ...
      {!isLoading && tab === 'requests' && (
        requests.length === 0
          ? <div style={{ backgroundColor: 'var(--card-bg)', borderRadius: 20, border: '1px solid rgb(var(--inv) / 0.08)', padding: 60, textAlign: 'center' }}>
              <p style={{ color: 'rgb(var(--inv) / 0.4)' }}>No prayer requests yet</p>
            </div>
          : <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
              {requests.map(r => {
                const sc = prayerStatusColor(r.status)
                return (
                  <div key={r.id} style={{ backgroundColor: 'var(--card-bg)', borderRadius: 20, border: '1px solid rgb(var(--inv) / 0.08)', overflow: 'hidden' }}>
                    ...
                    <div style={{ padding: 20 }}>
                      ...
                      <h3 style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: 15, margin: '0 0 4px' }}>{r.title}</h3>
                      <p style={{ color: 'rgb(var(--inv) / 0.4)', fontSize: 12, margin: '0 0 8px' }}>{r.anonymous ? 'Anonymous' : (r.requesterName || 'Member')} {r.createdAt ? '· ' + new Date(r.createdAt).toLocaleDateString() : ''}</p>
                      <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
                        {r.category && <span style={{ backgroundColor: 'rgb(var(--inv) / 0.06)', color: 'rgb(var(--inv) / 0.5)', borderRadius: 20, padding: '2px 8px', fontSize: 11 }}>{r.category}</span>}
                      ...
                      <div style={{ display: 'flex', gap: 6 }}>
                        {r.status === 'OPEN' && <button onClick={() => prayerStatus.mutate({ id: r.id, status: 'IN_PRAYER' })} style={{ backgroundColor: 'rgba(124,107,255,0.15)', border: 'none', color: 'var(--accent)', borderRadius: 8, padding: '5px 10px', cursor: 'pointer', fontSize: 11 }}>🕊️ In Prayer</button>}
                        ...
                        {r.status !== 'CLOSED' && <button onClick={() => prayerStatus.mutate({ id: r.id, status: 'CLOSED' })} style={{ backgroundColor: 'rgb(var(--inv) / 0.06)', border: 'none', color: 'rgb(var(--inv) / 0.5)', borderRadius: 8, padding: '5px 10px', cursor: 'pointer', fontSize: 11 }}>Close</button>}
                      </div>
                    </div>
                  </div>
                )
      ...
      {!isLoading && tab === 'testimonies' && (
        testimonies.length === 0
          ? <div style={{ backgroundColor: 'var(--card-bg)', borderRadius: 20, border: '1px solid rgb(var(--inv) / 0.08)', padding: 60, textAlign: 'center' }}>
              <p style={{ color: 'rgb(var(--inv) / 0.4)' }}>No testimonies yet</p>
            </div>
          : <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
              {testimonies.map(t => {
                const sc = testimonyStatusColor(t.status)
                return (
                  <div key={t.id} style={{ backgroundColor: 'var(--card-bg)', borderRadius: 20, border: '1px solid rgb(var(--inv) / 0.08)', overflow: 'hidden' }}>
                    <div style={{ height: 4, background: t.status === 'FEATURED' ? 'linear-gradient(135deg, #f59e0b, #ef4444)' : t.status === 'APPROVED' ? '#34d399' : t.status === 'REJECTED' ? '#f87171' : 'rgb(var(--inv) / 0.1)' }} />
                    <div style={{ padding: 20 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                      ...
                      <h3 style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: 15, margin: '0 0 4px' }}>{t.title}</h3>
                      <p style={{ color: 'rgb(var(--inv) / 0.4)', fontSize: 12, margin: '0 0 8px' }}>{t.anonymous ? 'Anonymous' : (t.authorName || 'Member')} {t.createdAt ? '· ' + new Date(t.createdAt).toLocaleDateString() : ''}</p>
                      {t.category && <span style={{ backgroundColor: 'rgb(var(--inv) / 0.06)', color: 'rgb(var(--inv) / 0.5)', borderRadius: 20, padding: '2px 8px', fontSize: 11, display: 'inline-block', marginBottom: 10 }}>{t.category}</span>}
      ...
      <Drawer open={testimonyDrawer} onClose={() => setTestimonyDrawer(false)} title="Share Testimony"
        ...
        <div style={{ backgroundColor: 'rgba(124,107,255,0.1)', borderRadius: 12, padding: '12px 16px' }}>
          <p style={{ color: 'var(--accent)', fontSize: 13, margin: 0 }}>ℹ️ Testimonies are reviewed before being published to the church community.</p>
        </div>
      </Drawer>
    </div>
  )
