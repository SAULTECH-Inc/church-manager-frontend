  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
      ...
      <button onClick={() => onChange(!value)}
        style={{ width: 44, height: 24, borderRadius: 12, border: 'none', cursor: 'pointer', position: 'relative', backgroundColor: value ? '#7c6bff' : 'rgba(255,255,255,0.15)', transition: 'background-color 0.2s' }}>
...
export function SettingsPage() {
  ...
  return (
    <div style={{ padding: '24px 32px', minHeight: '100vh', backgroundColor: '#131326' }}>
      <div className="mb-6">
        <h1 style={{ color: 'white', fontWeight: 700, fontSize: 26, margin: 0 }}>Settings</h1>
        <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 14, marginTop: 4 }}>Account · Organisation · Security</p>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr', gap: 20 }}>
        {/* Sidebar nav */}
        <div style={{ backgroundColor: '#13152e', borderRadius: 20, border: '1px solid rgba(255,255,255,0.08)', padding: 8, height: 'fit-content' }}>
          {NAV_ITEMS.map(item => (
            <button key={item.key} onClick={() => setActiveSection(item.key)}
              style={{
                ...
                color: activeSection === item.key ? '#7c6bff' : 'rgba(255,255,255,0.6)',
        ...
        {/* Content panel */}
        <div style={{ backgroundColor: '#13152e', borderRadius: 20, border: '1px solid rgba(255,255,255,0.08)', padding: 28 }}>

          {/* PROFILE */}
          {activeSection === 'profile' && (
            <div>
              <h2 style={{ color: 'white', fontWeight: 700, fontSize: 18, margin: '0 0 20px' }}>Profile Settings</h2>
              {profileForm.avatarUrl ? (
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24 }}>
                  <div style={{ width: 64, height: 64, borderRadius: 16, background: 'linear-gradient(135deg, #7c6bff, #6456e8)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24 }}>👤</div>
          ...
          {activeSection === 'branding' && (
            <div>
              <h2 style={{ color: 'white', fontWeight: 700, fontSize: 18, margin: '0 0 20px' }}>Organisation Branding</h2>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                    <input type="color" value={orgForm.primaryColor ?? '#7c6bff'} onChange={e => setOrgForm(f => ({ ...f, primaryColor: e.target.value }))}
                      style={{ width: 44, height: 44, borderRadius: 10, border: '1px solid rgba(255,255,255,0.1)', backgroundColor: 'transparent', cursor: 'pointer', padding: 2 }} />
          ...
          {activeSection === 'printouts' && (
            <div>
              <h2 style={{ color: 'white', fontWeight: 700, fontSize: 18, margin: '0 0 4px' }}>Printout Settings</h2>
              <p style={{ color: 'rgba(255,255,255,0.45)', fontSize: 13, margin: '0 0 24px' }}>Letterhead text and locale settings appear on all exported PDFs and printed documents.</p>
              <div style={{ marginBottom: 16 }}>
                ...
                <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: 12, marginTop: 4 }}>Appears at the top of every printed document. Supports line breaks.</p>
              </div>
              <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ backgroundColor: 'rgba(124,107,255,0.08)', border: '1px solid rgba(124,107,255,0.15)', borderRadius: 14, padding: '14px 18px', marginBottom: 24 }}>
                <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: 13, margin: 0 }}>
                  <strong style={{ color: '#a78bfa' }}>Logo & branding</strong> for printed documents is taken from the <button onClick={() => setActiveSection('branding')} style={{ background: 'none', border: 'none', color: '#7c6bff', cursor: 'pointer', padding: 0, fontSize: 13, fontWeight: 600, textDecoration: 'underline' }}>Branding</button> section (Logo URL).
                </p>
              </div>
          ...
          {activeSection === 'notifications' && (
            <div>
              <h2 style={{ color: 'white', fontWeight: 700, fontSize: 18, margin: '0 0 20px' }}>Notification Preferences</h2>
          ...
          {activeSection === 'security' && (
            <div>
              <h2 style={{ color: 'white', fontWeight: 700, fontSize: 18, margin: '0 0 20px' }}>Security Settings</h2>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, margin: '20px 0' }}>
                ...
                <div>
                  ...
                  <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 12, marginTop: 4 }}>Set to 0 to never expire</p>
                </div>
              </div>
              ...
              <div style={{ borderTop: '1px solid rgba(255,255,255,0.08)', marginTop: 28, paddingTop: 28 }}>
                <h3 style={{ color: 'white', fontWeight: 700, fontSize: 15, margin: '0 0 16px' }}>Change Password</h3>
                <div style={{ maxWidth: 360, display: 'flex', flexDirection: 'column' as const, gap: 14 }}>
                  ...
                  <div>
                    ...
                    <input type="password" value={pwForm.confirmPassword} onChange={e => setPwForm(f => ({ ...f, confirmPassword: e.target.value }))}
                      style={{ ...inputStyle, borderColor: pwForm.confirmPassword && !pwMatch ? '#f87171' : 'rgba(255,255,255,0.10)' }} />
          ...
          {activeSection === 'integrations' && (
            <div>
              <h2 style={{ color: 'white', fontWeight: 700, fontSize: 18, margin: '0 0 20px' }}>Integrations</h2>
              {[
                ...
                <div key={intg.name} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                    <span style={{ fontSize: 22 }}>{intg.icon}</span>
                    <div>
                      <p style={{ color: 'white', fontWeight: 600, fontSize: 14, margin: 0 }}>{intg.name}</p>
                      <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 13, margin: 0 }}>{intg.desc}</p>
                    </div>
                  </div>
          ...
          {activeSection === 'billing' && (
            <div>
              <h2 style={{ color: 'white', fontWeight: 700, fontSize: 18, margin: '0 0 20px' }}>Billing & Plan</h2>
              <div style={{ backgroundColor: 'rgba(124,107,255,0.1)', border: '1px solid rgba(124,107,255,0.2)', borderRadius: 16, padding: '20px 24px', marginBottom: 24 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    ...
                    <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 14, margin: '4px 0 0' }}>All features · Unlimited members · Priority support</p>
                  </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14, marginBottom: 24 }}>
                {[
                  ...
                  <div key={s.label} style={{ backgroundColor: 'rgba(255,255,255,0.04)', borderRadius: 12, padding: 14 }}>
                    <p style={{ color: 'white', fontWeight: 700, fontSize: 16, margin: 0 }}>{s.value}</p>
                    <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 12, margin: '4px 0 0' }}>{s.label}</p>
                  </div>
                ))}
              </div>
