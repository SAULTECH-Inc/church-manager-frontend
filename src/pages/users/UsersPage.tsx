...
const roleColors: Record<string, { color: string; bg: string }> = {
  ...
  MEMBER: { color: 'rgba(255,255,255,0.5)', bg: 'rgba(255,255,255,0.08)' },
}
  ...
  return (
    <div style={{ padding: '24px 32px', minHeight: '100vh', backgroundColor: '#131326' }}>
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 style={{ color: 'white', fontWeight: 700, fontSize: 26, margin: 0 }}>User Accounts</h1>
          <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 14, marginTop: 4 }}>System Users · Roles · Permissions</p>
        </div>
        ...
        <div style={{ backgroundColor: '#13152e', borderRadius: 20, border: '1px solid rgba(255,255,255,0.08)', overflow: 'hidden' }}>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {users.length === 0 && <tr><td colSpan={7} style={{ textAlign: 'center', padding: 40, color: 'rgba(255,255,255,0.4)' }}>No users yet</td></tr>}
                {users.map(u => {
                  const rc = roleColors[u.role] ?? roleColors.MEMBER
                  return (
                    <tr key={u.id} className="border-b" style={{ borderColor: 'rgba(255,255,255,0.04)', backgroundColor: selected.has(u.id) ? 'rgba(124,107,255,0.05)' : undefined }}>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          <div style={{ width: 34, height: 34, borderRadius: 10, background: 'linear-gradient(135deg, #7c6bff, #6456e8)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: 12, fontWeight: 700, flexShrink: 0 }}>
                            {getInitials(u.displayName ?? u.email)}
                          </div>
                          <div>
                            <p style={{ color: 'white', fontWeight: 600, margin: 0, fontSize: 14 }}>{u.displayName || '—'}</p>
                            <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 12, margin: 0 }}>{u.email}</p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-5 py-4" style={{ color: 'rgba(255,255,255,0.6)', fontSize: 13 }}>{u.memberName || <span style={{ color: 'rgba(255,255,255,0.3)', fontStyle: 'italic' }}>Unlinked</span>}</td>
                      <td className="px-5 py-4">
                        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13 }}>
                          <span style={{ width: 7, height: 7, borderRadius: '50%', backgroundColor: u.active ? '#10b981' : 'rgba(255,255,255,0.3)', display: 'inline-block', boxShadow: u.active ? '0 0 6px #10b981' : 'none' }} />
                          <span style={{ color: u.active ? '#34d399' : 'rgba(255,255,255,0.4)' }}>{u.active ? 'Active' : 'Inactive'}</span>
                        </span>
                      </td>
                      <td className="px-5 py-4" style={{ color: 'rgba(255,255,255,0.4)', fontSize: 13 }}>{u.lastLogin ? new Date(u.lastLogin).toLocaleDateString() : 'Never'}</td>
                      <td className="px-5 py-4">
                        <div className="flex gap-1">
                          <button onClick={() => openEdit(u)} style={{ background: 'rgba(255,255,255,0.06)', border: 'none', color: 'rgba(255,255,255,0.6)', borderRadius: 8, width: 30, height: 30, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Edit2 size={13} /></button>
      ...
      <Drawer open={resetDrawer} onClose={() => { setResetDrawer(false); setResetForm({ newPassword: '', confirmPassword: '' }); setResetUserId('') }} title="Reset Password"
        ...
        <div>
          ...
          <input type="password" value={resetForm.confirmPassword} onChange={e => setResetForm(f => ({ ...f, confirmPassword: e.target.value }))}
            style={{ ...inputStyle, borderColor: resetForm.confirmPassword && !passwordsMatch ? '#f87171' : 'rgba(255,255,255,0.10)' }} />
