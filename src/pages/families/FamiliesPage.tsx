...
export function FamiliesPage() {
  ...
  const deleteFamilyMutation = useMutation({
    mutationFn: (id: string) => api.post(`/api/families/${id}/delete`),
  ...
  if (selectedFamilyId && !familyDetail) {
    return (
      <div style={{ padding: '24px 32px', minHeight: '100vh', backgroundColor: 'var(--page-bg)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16 }}>
        <p style={{ color: 'rgb(var(--inv) / 0.4)', fontSize: 15 }}>{detailError ? 'Failed to load family details.' : 'Loading family details...'}</p>
        <button onClick={() => setSelectedFamilyId(null)} style={{ background: 'rgb(var(--inv) / 0.06)', border: 'none', color: 'rgb(var(--inv) / 0.5)', borderRadius: 10, padding: '8px 16px', cursor: 'pointer', fontSize: 14 }}>← Back to Families</button>
      </div>
    )
  }
    ...
    return (
      <div style={{ padding: '24px 32px', minHeight: '100vh', backgroundColor: 'var(--page-bg)' }}>
        <div className="flex items-center gap-4 mb-6">
          <button onClick={() => setSelectedFamilyId(null)} style={{ background: 'rgb(var(--inv) / 0.06)', border: 'none', color: 'rgb(var(--inv) / 0.6)', borderRadius: 10, width: 36, height: 36, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <ArrowLeft size={16} />
          </button>
          <div>
            <h1 style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: 26, margin: 0 }}>{familyDetail.name}</h1>
            <p style={{ color: 'rgb(var(--inv) / 0.5)', fontSize: 14, marginTop: 4 }}>{familyDetail.headName ? `Head of House: ${familyDetail.headName}` : 'Household'}</p>
          </div>
        ...
        <div style={{ backgroundColor: 'var(--card-bg)', borderRadius: 20, border: '1px solid rgb(var(--inv) / 0.08)', overflow: 'hidden' }}>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {roster.length === 0 && <tr><td colSpan={4} style={{ textAlign: 'center', padding: 40, color: 'rgb(var(--inv) / 0.4)' }}>No household members yet</td></tr>}
                {roster.map(r => (
                  <tr key={r.memberId} className="border-b" style={{ borderColor: 'rgb(var(--inv) / 0.04)' }}>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        <div style={{ width: 32, height: 32, borderRadius: 10, background: 'linear-gradient(135deg, var(--accent), var(--accent-dark))', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-primary)', fontSize: 11, fontWeight: 700 }}>
                          {getInitials(r.memberName)}
                        </div>
                        <span style={{ color: 'var(--text-primary)', fontWeight: 500 }}>{r.memberName}</span>
                      </div>
                    </td>
                    <td className="px-5 py-4"><span style={{ backgroundColor: 'rgba(124,107,255,0.15)', color: 'var(--accent)', borderRadius: 20, padding: '3px 10px', fontSize: 12, fontWeight: 600 }}>{r.relationship}</span></td>
                    <td className="px-5 py-4" style={{ color: 'rgb(var(--inv) / 0.5)', fontSize: 13 }}>{r.phone || r.email || '—'}</td>
                    <td className="px-5 py-4">
  ...
  return (
    <div style={{ padding: '24px 32px', minHeight: '100vh', backgroundColor: 'var(--page-bg)' }}>
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: 26, margin: 0 }}>Family Directory</h1>
          <p style={{ color: 'rgb(var(--inv) / 0.5)', fontSize: 14, marginTop: 4 }}>Household Registry · Family Groups</p>
        </div>
      ...
      {!isLoading && families.length === 0 && (
        <div style={{ backgroundColor: 'var(--card-bg)', borderRadius: 20, border: '1px solid rgb(var(--inv) / 0.08)', padding: 60, textAlign: 'center' }}>
          <Users size={32} style={{ color: 'var(--accent)', margin: '0 auto 12px' }} />
          <p style={{ color: 'var(--text-primary)', fontWeight: 600 }}>No families registered</p>
        ...
        <div style={{ backgroundColor: 'var(--card-bg)', borderRadius: 20, border: '1px solid rgb(var(--inv) / 0.08)', overflow: 'hidden' }}>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {families.map(f => (
                  <tr key={f.id} className="border-b" style={{ borderColor: 'rgb(var(--inv) / 0.04)' }}>
                    <td className="px-5 py-4" style={{ color: 'var(--text-primary)', fontWeight: 600, fontSize: 15 }}>{f.name}</td>
                    <td className="px-5 py-4" style={{ color: 'rgb(var(--inv) / 0.6)', fontSize: 13 }}>{f.headName || '—'}</td>
                    <td className="px-5 py-4"><span style={{ backgroundColor: 'rgba(124,107,255,0.15)', color: 'var(--accent)', borderRadius: 20, padding: '3px 14px', fontSize: 13, fontWeight: 700 }}>{f.rosterSize ?? 0}</span></td>
                    <td className="px-5 py-4">
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <button onClick={() => setSelectedFamilyId(f.id)}
                          style={{ backgroundColor: 'rgba(124,107,255,0.15)', border: 'none', color: 'var(--accent)', borderRadius: 8, padding: '6px 14px', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>
                          Manage
                        </button>
                        <button onClick={() => { if (confirm(`Delete "${f.name}"?`)) deleteFamilyMutation.mutate(f.id) }}
                          style={{ background: 'rgba(239,68,68,0.1)', border: 'none', color: '#f87171', borderRadius: 8, width: 30, height: 30, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          <Trash2 size={13} />
                        </button>
                      </div>
