...
import { Plus, Trash2, X, Check, Download, ChevronDown, Pencil } from 'lucide-react'
...
export function ExpensesPage() {
  ...
  const [editForm, setEditForm] = useState({ categoryId: '', recipientOrVendor: '', amount: '', paymentMethod: 'CASH', expenseDate: '', description: '' })
  ...
  const editMutation = useMutation({
    mutationFn: () => api.post(`/api/expenses/${editExpense?.id}/edit?${toQS(editForm)}`),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['expenses'] }); setEditExpense(null) },
  })
  ...
  const openEdit = (e: Expense) => {
    ...
    setEditForm({
      categoryId: '',
      recipientOrVendor: e.recipientOrVendor,
  ...
  return (
    <div style={{ padding: '24px 32px', minHeight: '100vh', backgroundColor: 'var(--page-bg)' }}>
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: 26, margin: 0 }}>Expenses</h1>
          <p style={{ color: 'rgb(var(--inv) / 0.5)', fontSize: 14, marginTop: 4 }}>Expense Tracking · Categories · Budgets</p>
        </div>
        ...
        <div style={{ backgroundColor: 'var(--card-bg)', borderRadius: 20, border: '1px solid rgb(var(--inv) / 0.08)', overflow: 'hidden' }}>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {expenses.length === 0 && <tr><td colSpan={7} style={{ textAlign: 'center', padding: 40, color: 'rgb(var(--inv) / 0.4)' }}>No expenses logged yet</td></tr>}
                {expenses.map(e => {
                  const sc = statusColors[e.status] ?? { color: 'rgb(var(--inv) / 0.5)', bg: 'rgb(var(--inv) / 0.06)' }
                  return (
                    <tr key={e.id} className="border-b" style={{ borderColor: 'rgb(var(--inv) / 0.04)' }}>
                      <td className="px-5 py-4"><span style={{ backgroundColor: 'rgba(124,107,255,0.15)', color: 'var(--accent)', borderRadius: 20, padding: '3px 10px', fontSize: 12, fontWeight: 600 }}>{e.categoryName ?? '—'}</span></td>
                      <td className="px-5 py-4" style={{ color: 'var(--text-primary)', fontWeight: 500 }}>{e.recipientOrVendor}</td>
                      <td className="px-5 py-4" style={{ color: '#f87171', fontWeight: 700 }}>{fmt(e.amount)}</td>
                      <td className="px-5 py-4" style={{ color: 'rgb(var(--inv) / 0.6)', fontSize: 13 }}>{e.paymentMethod}</td>
                      <td className="px-5 py-4" style={{ color: 'rgb(var(--inv) / 0.6)', fontSize: 13 }}>{new Date(e.expenseDate).toLocaleDateString()}</td>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex gap-2">
                          ...
                          <button onClick={() => openEdit(e)}
                            style={{ backgroundColor: 'rgba(124,107,255,0.1)', border: 'none', color: '#a5b4fc', borderRadius: 8, width: 30, height: 30, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      {!isLoading && tab === 'categories' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
          {categories.length === 0 && <div style={{ gridColumn: '1/-1', backgroundColor: 'var(--card-bg)', borderRadius: 20, border: '1px solid rgb(var(--inv) / 0.08)', padding: 60, textAlign: 'center' }}>
            <p style={{ color: 'rgb(var(--inv) / 0.4)' }}>No categories yet</p>
            <button onClick={() => setCatDrawer(true)} style={{ marginTop: 12, ...gradientBtn }}>Add Category</button>
          </div>}
          {categories.map(c => (
            <div key={c.id} style={{ backgroundColor: 'var(--card-bg)', borderRadius: 20, border: '1px solid rgb(var(--inv) / 0.08)', padding: 20 }}>
              <div style={{ marginBottom: 12 }}>
                <h3 style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: 16, margin: 0 }}>{c.name}</h3>
              </div>
              {c.description && <p style={{ color: 'rgb(var(--inv) / 0.5)', fontSize: 13, margin: '0 0 12px' }}>{c.description}</p>}
              <div style={{ display: 'flex', gap: 12 }}>
                <div style={{ flex: 1, backgroundColor: 'rgb(var(--inv) / 0.04)', borderRadius: 10, padding: '10px 12px' }}>
                  <p style={{ color: 'rgb(var(--inv) / 0.4)', fontSize: 11, margin: '0 0 4px' }}>EXPENSES</p>
                  <p style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: 18, margin: 0 }}>{c.expenseCount ?? 0}</p>
                </div>
                <div style={{ flex: 1, backgroundColor: 'rgb(var(--inv) / 0.04)', borderRadius: 10, padding: '10px 12px' }}>
                  <p style={{ color: 'rgb(var(--inv) / 0.4)', fontSize: 11, margin: '0 0 4px' }}>ALLOCATED</p>
                  <p style={{ color: 'var(--accent)', fontWeight: 700, fontSize: 18, margin: 0 }}>{fmt(c.allocatedTotal ?? 0)}</p>
                </div>
              </div>
            </div>
          ))}
      ...
      {!isLoading && tab === 'budgets' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
          {budgets.length === 0 && <div style={{ gridColumn: '1/-1', backgroundColor: 'var(--card-bg)', borderRadius: 20, border: '1px solid rgb(var(--inv) / 0.08)', padding: 60, textAlign: 'center' }}>
            <p style={{ color: 'rgb(var(--inv) / 0.4)' }}>No budgets allocated yet</p>
          ...
          {budgets.map(b => {
            ...
            return (
              <div key={b.id} style={{ backgroundColor: 'var(--card-bg)', borderRadius: 20, border: '1px solid rgb(var(--inv) / 0.08)', padding: 20 }}>
                <p style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: 16, margin: '0 0 4px' }}>{b.categoryName ?? 'Category'}</p>
                <p style={{ color: 'rgb(var(--inv) / 0.4)', fontSize: 12, margin: '0 0 16px' }}>{new Date(b.startDate).toLocaleDateString()} – {new Date(b.endDate).toLocaleDateString()}</p>
                <div style={{ marginBottom: 12 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                    <span style={{ color: 'rgb(var(--inv) / 0.5)', fontSize: 12 }}>Spent {fmt(b.spentAmount)}</span>
                    <span style={{ color: 'rgb(var(--inv) / 0.5)', fontSize: 12 }}>{pct.toFixed(0)}%</span>
                  </div>
                  <div style={{ height: 6, borderRadius: 3, backgroundColor: 'rgb(var(--inv) / 0.08)' }}>
                    <div style={{ height: '100%', borderRadius: 3, backgroundColor: barColor, width: `${pct}%` }} />
                  </div>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <div><p style={{ color: 'rgb(var(--inv) / 0.4)', fontSize: 11, margin: 0 }}>ALLOCATED</p><p style={{ color: 'var(--text-primary)', fontWeight: 600, fontSize: 15, margin: 0 }}>{fmt(b.allocatedAmount)}</p></div>
                  <div style={{ textAlign: 'right' }}><p style={{ color: 'rgb(var(--inv) / 0.4)', fontSize: 11, margin: 0 }}>REMAINING</p><p style={{ color: b.remainingAmount >= 0 ? '#34d399' : '#f87171', fontWeight: 600, fontSize: 15, margin: 0 }}>{fmt(b.remainingAmount)}</p></div>
                </div>
              </div>
            )
          })}
      ...
      <Drawer open={!!editExpense} onClose={() => setEditExpense(null)} title="Edit Expense"
        footer={<>
          <button onClick={() => setEditExpense(null)} style={outlineBtn}>Cancel</button>
          <button onClick={() => editMutation.mutate()} disabled={!editForm.recipientOrVendor || !editForm.amount || editMutation.isPending} style={gradientBtn}>{editMutation.isPending ? 'Saving...' : 'Save Changes'}</button>
        </>}>
        ...
        <div><label style={labelStyle}>RECIPIENT / VENDOR <span style={{ color: '#f87171' }}>*</span></label>
          <input type="text" value={editForm.recipientOrVendor} onChange={e => setEditForm(f => ({ ...f, recipientOrVendor: e.target.value }))} style={inputStyle} /></div>
        <div><label style={labelStyle}>AMOUNT <span style={{ color: '#f87171' }}>*</span></label>
          <input type="number" min="0" step="0.01" value={editForm.amount} onChange={e => setEditForm(f => ({ ...f, amount: e.target.value }))} style={inputStyle} /></div>
        <div><label style={labelStyle}>PAYMENT METHOD</label>
          <select value={editForm.paymentMethod} onChange={e => setEditForm(f => ({ ...f, paymentMethod: e.target.value }))} style={inputStyle}>
            {['CASH', 'BANK_TRANSFER', 'CARD', 'CHEQUE', 'ONLINE', 'OTHER'].map(m => <option key={m}>{m}</option>)}
          </select></div>
        <div><label style={labelStyle}>DATE</label>
          <input type="date" value={editForm.expenseDate} onChange={e => setEditForm(f => ({ ...f, expenseDate: e.target.value }))} style={inputStyle} /></div>
        <div><label style={labelStyle}>DESCRIPTION</label>
          <textarea rows={3} value={editForm.description} onChange={e => setEditForm(f => ({ ...f, description: e.target.value }))} style={{ ...inputStyle, resize: 'vertical' as const }} /></div>
      </Drawer>
      ...
      <Drawer open={budDrawer} onClose={() => setBudDrawer(false)} title="Allocate Budget"
        footer={<>
          ...
          <button onClick={() => budMutation.mutate()} disabled={!budForm.categoryId || !budForm.allocatedAmount || !budForm.startDate || !budForm.endDate || budMutation.isPending} style={gradientBtn}>{budMutation.isPending ? 'Saving...' : 'Allocate Budget'}</button>
        </>}>
        <div><label style={labelStyle}>CATEGORY <span style={{ color: '#f87171' }}>*</span></label>
