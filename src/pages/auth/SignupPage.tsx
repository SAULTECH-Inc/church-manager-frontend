...
function Field({ label, error, children }: { label: string; error?: string; children: React.ReactNode }) {
  return (
    <div>
      <label className="block text-xs font-semibold tracking-widest mb-1.5" style={{ color: 'rgba(255,255,255,0.5)' }}>{label}</label>
      {children}
...
export function SignupPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center py-12 px-4 relative overflow-hidden" style={{ backgroundColor: '#131326' }}>
      {/* Decorative blobs */}
      ...
      <div className="w-full max-w-xl relative z-10">
        <div style={CARD_STYLE} className="p-10">
          ...
          <div className="flex flex-col items-center mb-8">
            ...
            <h1 className="text-2xl font-bold text-white">Get Started with ChurchOS</h1>
            <p className="text-sm mt-1.5" style={{ color: 'rgba(255,255,255,0.5)' }}>Set up your multi-tenant global church platform</p>
          </div>
          ...
          <form onSubmit={handleSubmit(d => signupMutation.mutate(d))} className="space-y-6">
            {/* Church details */}
            <div>
              <p className="text-xs font-bold tracking-widest mb-4" style={{ color: '#7c6bff' }}>CHURCH ORGANIZATION DETAILS</p>
                ...
                <Field label="SUBDOMAIN" error={errors.subdomain?.message}>
                  <div className="flex">
                    <input {...register('subdomain')} placeholder="grace" style={{ ...INPUT_STYLE, borderRadius: '14px 0 0 14px' }}
                      onFocus={e => (e.currentTarget.style.borderColor = '#7c6bff')}
                      onBlur={e => (e.currentTarget.style.borderColor = '#252855')} />
                    <span className="flex items-center px-3 text-xs font-medium whitespace-nowrap" style={{ backgroundColor: '#252643', border: '1px solid #252855', borderLeft: 'none', borderRadius: '0 14px 14px 0', color: 'rgba(255,255,255,0.5)' }}>
                      .churchos.org
                    </span>
                  </div>
                </Field>
            ...
            {/* Admin credentials */}
            <div>
              <p className="text-xs font-bold tracking-widest mb-4" style={{ color: '#7c6bff' }}>ADMINISTRATOR ACCOUNT</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <button
              ...
              style={{ background: 'linear-gradient(135deg, #7c6bff, #6456e8)', color: '#131326', boxShadow: '0 8px 20px rgba(100,86,232,0.3)' }}
            >
          ...
          <p className="text-center mt-6 text-sm" style={{ color: 'rgba(255,255,255,0.45)' }}>
            ...
            <Link to="/login" className="font-semibold" style={{ color: '#7c6bff' }}>Sign In</Link>
          </p>
        </div>
      </div>
    </div>
