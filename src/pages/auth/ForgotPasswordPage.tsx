          ...
          {mutation.isSuccess ? (
            <div style={{ textAlign: 'center' }}>
              ...
              <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 14, lineHeight: 1.6 }}>
                If <strong style={{ color: 'white' }}>{email}</strong> is registered, we've sent a password reset link. It expires in 1 hour.
              </p>
              <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: 12, marginTop: 12 }}>
                Didn't receive it? Check your spam folder.
              </p>
              <Link to="/login" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, color: 'rgba(255,255,255,0.5)', fontSize: 13, marginTop: 24, textDecoration: 'none' }}>
            ...
            <>
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div style={{ position: 'relative' }}>
                  <Mail size={16} style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', color: 'rgba(255,255,255,0.35)', pointerEvents: 'none' }} />
                  <input
                    type="email"
                    value={email}
                ...
                <button type="submit" disabled={mutation.isPending || !email.trim()}
                  ...
                  style={{ background: 'linear-gradient(135deg, #7c6bff, #6456e8)', border: 'none', cursor: mutation.isPending ? 'not-allowed' : 'pointer', opacity: mutation.isPending ? 0.7 : 1 }}>
