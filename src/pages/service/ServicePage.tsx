...
const SERVICE_TYPE_LABELS: Record<string, { label: string; color: string; bg: string }> = {
  SUNDAY_SERVICE: { label: 'Sunday Service', color: 'var(--accent)', bg: 'rgba(124,107,255,0.15)' },
  ...
  return (
    <div style={{ padding: '24px 32px', minHeight: '100vh', backgroundColor: 'var(--page-bg)' }}>
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: 26, margin: 0 }}>Service Management</h1>
          <p style={{ color: 'rgb(var(--inv) / 0.5)', fontSize: 14, marginTop: 4 }}>Schedule · Sermons · Series</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 16, marginBottom: 24 }}>
        {[
          { label: 'Total Services', value: stats.totalServices ?? schedules.length, color: 'var(--accent)' },
      ...
      {!isLoading && tab === 'schedule' && (
        schedules.length === 0
          ? <div style={{ backgroundColor: 'var(--card-bg)', borderRadius: 20, border: '1px solid rgb(var(--inv) / 0.08)', padding: 60, textAlign: 'center' }}>
              <Calendar size={32} style={{ color: 'var(--accent)', margin: '0 auto 12px' }} />
              <p style={{ color: 'var(--text-primary)', fontWeight: 600 }}>No services scheduled</p>
              <button onClick={() => setScheduleDrawer(true)} style={{ marginTop: 12, ...gradientBtn }}>Schedule Service</button>
            </div>
          : <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
              {schedules.map(s => {
                const stype = SERVICE_TYPE_LABELS[s.serviceType] ?? { label: s.serviceType, color: 'var(--accent)', bg: 'rgba(124,107,255,0.15)' }
                return (
                  <div key={s.id} style={{ backgroundColor: 'var(--card-bg)', borderRadius: 20, border: '1px solid rgb(var(--inv) / 0.08)', padding: 20 }}>
                    ...
                    <h3 style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: 16, margin: '0 0 8px' }}>{s.title}</h3>
                    <p style={{ color: 'rgb(var(--inv) / 0.5)', fontSize: 13, margin: '0 0 3px' }}>📅 {new Date(s.serviceDate).toLocaleDateString()}</p>
                    {(s.startTime || s.endTime) && <p style={{ color: 'rgb(var(--inv) / 0.5)', fontSize: 13, margin: '0 0 3px' }}>🕐 {s.startTime}{s.endTime ? ` – ${s.endTime}` : ''}</p>}
                    {s.theme && <p style={{ color: 'rgb(var(--inv) / 0.5)', fontSize: 13, margin: '0 0 3px' }}>✨ {s.theme}</p>}
                    {s.preacherName && <p style={{ color: 'rgb(var(--inv) / 0.5)', fontSize: 13, margin: 0 }}>🎤 {s.preacherName}</p>}
                  </div>
                )
              })}
            </div>
        ...
        <div style={{ backgroundColor: 'var(--card-bg)', borderRadius: 20, border: '1px solid rgb(var(--inv) / 0.08)', overflow: 'hidden' }}>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {sermons.length === 0 && <tr><td colSpan={6} style={{ textAlign: 'center', padding: 40, color: 'rgb(var(--inv) / 0.4)' }}>No sermons added yet</td></tr>}
                {sermons.map(s => (
                  <tr key={s.id} className="border-b" style={{ borderColor: 'rgb(var(--inv) / 0.04)' }}>
                    <td className="px-5 py-4"><p style={{ color: 'var(--text-primary)', fontWeight: 600, margin: 0 }}>{s.title}</p>{s.summary && <p style={{ color: 'rgb(var(--inv) / 0.4)', fontSize: 12, margin: '2px 0 0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 200 }}>{s.summary}</p>}</td>
                    <td className="px-5 py-4">{s.series ? <span style={{ backgroundColor: 'rgba(139,92,246,0.15)', color: '#8b5cf6', borderRadius: 20, padding: '3px 10px', fontSize: 12, fontWeight: 600 }}>{s.series}</span> : <span style={{ color: 'rgb(var(--inv) / 0.3)' }}>—</span>}</td>
                    <td className="px-5 py-4" style={{ color: 'rgb(var(--inv) / 0.6)', fontSize: 13 }}>{new Date(s.sermonDate).toLocaleDateString()}</td>
                    <td className="px-5 py-4" style={{ color: 'rgb(var(--inv) / 0.6)', fontSize: 13 }}>{s.preacherName || '—'}</td>
                    <td className="px-5 py-4" style={{ color: 'rgb(var(--inv) / 0.6)', fontSize: 13 }}>{s.scriptureRef || '—'}</td>
