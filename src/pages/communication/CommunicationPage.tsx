...
export function CommunicationPage() {
  ...
  const kpis = [
    { label: 'Total Messages', value: stats.total ?? messages.length, color: 'var(--accent)' },
  ...
  return (
    <div style={{ padding: '24px 32px', minHeight: '100vh', backgroundColor: 'var(--page-bg)' }}>
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: 26, margin: 0 }}>Communication</h1>
          <p style={{ color: 'rgb(var(--inv) / 0.5)', fontSize: 14, marginTop: 4 }}>Messages · Email · SMS · Notifications</p>
        </div>
      ...
      <div style={{ backgroundColor: 'var(--card-bg)', borderRadius: 20, border: '1px solid rgb(var(--inv) / 0.08)', overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '16px 20px', borderBottom: '1px solid rgb(var(--inv) / 0.06)' }}>
          {['ALL', 'SENT', 'DRAFT', 'FAILED'].map(s => (
            <button key={s} onClick={() => setStatusFilter(s)}
              style={{ padding: '6px 14px', borderRadius: 8, border: 'none', cursor: 'pointer', fontWeight: 600, fontSize: 13, backgroundColor: statusFilter === s ? '#7c6bff' : 'rgb(var(--inv) / 0.06)', color: statusFilter === s ? 'white' : 'rgb(var(--inv) / 0.5)' }}>
              {s}
            </button>
          ))}
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {isLoading && <tr><td colSpan={7} style={{ textAlign: 'center', padding: 40, color: 'rgb(var(--inv) / 0.4)' }}>Loading...</td></tr>}
              {!isLoading && filtered.length === 0 && <tr><td colSpan={7} style={{ textAlign: 'center', padding: 40, color: 'rgb(var(--inv) / 0.4)' }}>No messages yet</td></tr>}
              {filtered.map(m => {
                const sc = msgStatusColor(m.status)
                return (
                  <tr key={m.id} className="border-b" style={{ borderColor: 'rgb(var(--inv) / 0.04)' }}>
                    <td className="px-5 py-4">
                      <p style={{ color: 'var(--text-primary)', fontWeight: 600, margin: 0 }}>{m.subject}</p>
                      {m.body && <RichTextDisplay html={m.body} clamp={2} style={{ color: 'rgb(var(--inv) / 0.4)', fontSize: 12, margin: '2px 0 0', maxWidth: 220 }} />}
                    </td>
                    <td className="px-5 py-4"><span style={{ backgroundColor: 'rgb(var(--inv) / 0.06)', color: 'var(--text-primary)', borderRadius: 20, padding: '3px 10px', fontSize: 12 }}>{channelIcon(m.channel)} {m.channel}</span></td>
                    <td className="px-5 py-4" style={{ color: 'rgb(var(--inv) / 0.6)', fontSize: 13 }}>{m.audienceFilter || 'ALL'}</td>
                    <td className="px-5 py-4" style={{ color: 'rgb(var(--inv) / 0.6)', fontSize: 13 }}>{m.recipientCount ?? '—'}</td>
                    ...
                    <td className="px-5 py-4" style={{ color: 'rgb(var(--inv) / 0.6)', fontSize: 13 }}>{m.sentAt ? new Date(m.sentAt).toLocaleDateString() : m.createdAt ? new Date(m.createdAt).toLocaleDateString() : '—'}</td>
                    <td className="px-5 py-4">
      ...
      <Drawer open={composeDrawer} onClose={() => setComposeDrawer(false)} title="Compose Message"
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
            {(['EMAIL', 'SMS', 'IN_APP'] as const).map(ch => (
              <button key={ch} onClick={() => setForm(f => ({ ...f, channel: ch }))}
                style={{ padding: '12px 8px', borderRadius: 12, border: `1px solid ${form.channel === ch ? '#7c6bff' : 'rgb(var(--inv) / 0.1)'}`, cursor: 'pointer', backgroundColor: form.channel === ch ? 'rgba(124,107,255,0.15)' : 'rgb(var(--inv) / 0.04)', color: form.channel === ch ? '#7c6bff' : 'rgb(var(--inv) / 0.6)', fontSize: 13, fontWeight: 600, textAlign: 'center' }}>
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
            ...
            <span style={{ color: 'rgb(var(--inv) / 0.4)', fontSize: 12 }}>{charCount}{form.channel === 'SMS' && ` · ${smsCredits} SMS credit${smsCredits !== 1 ? 's' : ''}`}</span>
          </div>
        ...
        <div style={{ backgroundColor: 'rgba(124,107,255,0.1)', borderRadius: 12, padding: '12px 16px' }}>
          <p style={{ color: 'var(--accent)', fontSize: 13, margin: 0 }}>ℹ️ Recipients are resolved from the church directory based on your audience selection at send time.</p>
        </div>
      </Drawer>
    </div>
  )
