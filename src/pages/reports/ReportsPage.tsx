  ...
  const ExportDropdown = ({ label, endpoint }: { label: string; endpoint: string }) => (
    <div style={{ position: 'relative' }}>
      <button onClick={() => setExportOpen(exportOpen === label ? null : label)}
        style={{ display: 'flex', alignItems: 'center', gap: 6, backgroundColor: 'rgb(var(--inv) / 0.06)', border: '1px solid rgb(var(--inv) / 0.1)', color: 'var(--text-primary)', borderRadius: 10, padding: '8px 14px', cursor: 'pointer', fontSize: 13 }}>
        {label} <ChevronDown size={13} />
      </button>
      {exportOpen === label && (
        <div style={{ position: 'absolute', top: '110%', right: 0, backgroundColor: 'var(--drawer-bg)', border: '1px solid rgb(var(--inv) / 0.08)', borderRadius: 12, zIndex: 20, minWidth: 120, overflow: 'hidden' }}
          ...
          {['xlsx', 'csv', 'pdf'].map(fmt => (
            <button key={fmt} onClick={() => { window.location.href = `/api/reports/export/${endpoint}?format=${fmt}`; setExportOpen(null) }}
              style={{ display: 'block', width: '100%', textAlign: 'left', padding: '9px 14px', background: 'none', border: 'none', color: 'rgb(var(--inv) / 0.8)', cursor: 'pointer', fontSize: 13, textTransform: 'uppercase' }}
  ...
  return (
    <div style={{ padding: '24px 32px', minHeight: '100vh', backgroundColor: 'var(--page-bg)' }}>
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: 26, margin: 0 }}>Reports & Analytics</h1>
          <p style={{ color: 'rgb(var(--inv) / 0.5)', fontSize: 14, marginTop: 4 }}>Financial Overview · Membership · Trends</p>
        </div>
      ...
      {isLoading && <div style={{ backgroundColor: 'var(--card-bg)', borderRadius: 20, border: '1px solid rgb(var(--inv) / 0.08)', padding: 40, textAlign: 'center' }}><p style={{ color: 'rgb(var(--inv) / 0.4)' }}>Loading report data...</p></div>}
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 16, marginBottom: 24 }}>
            {[
              ...
              { label: 'Total Members', value: mem.total.toLocaleString(), color: 'var(--accent)' },
          ...
          {/* Trend Chart */}
          <div style={{ backgroundColor: 'var(--card-bg)', borderRadius: 20, border: '1px solid rgb(var(--inv) / 0.08)', padding: 24, marginBottom: 24 }}>
            <h2 style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: 16, margin: '0 0 20px' }}>12-Month Financial Trend</h2>
            <div style={{ display: 'flex', gap: 16, marginBottom: 8 }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'rgb(var(--inv) / 0.6)', fontSize: 12 }}><span style={{ width: 24, height: 2, backgroundColor: '#6456e8', display: 'inline-block' }} />Income</span>
              <span style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'rgb(var(--inv) / 0.6)', fontSize: 12 }}><span style={{ width: 24, height: 2, backgroundColor: '#a78bfa', borderBottom: '2px dashed #a78bfa', display: 'inline-block' }} />Expenses</span>
            </div>
            <svg width="100%" height={trendH + 40} viewBox={`0 0 ${trend.labels.length * 60} ${trendH + 40}`} preserveAspectRatio="none">
              ...
              {trend.labels.map((l, i) => (
                <text key={l} x={i * 60 + 30} y={trendH + 30} textAnchor="middle" fill="rgb(var(--inv) / 0.4)" fontSize="11">{l}</text>
              ))}
            </svg>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginBottom: 24 }}>
            {/* Income Breakdown */}
            <div style={{ backgroundColor: 'var(--card-bg)', borderRadius: 20, border: '1px solid rgb(var(--inv) / 0.08)', padding: 24 }}>
              <h2 style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: 16, margin: '0 0 16px' }}>Income by Type</h2>
              {cb.length === 0 && <p style={{ color: 'rgb(var(--inv) / 0.4)', fontSize: 14 }}>No data</p>}
              {cb.map(item => (
                <div key={item.type} style={{ marginBottom: 14 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                    <span style={{ color: 'var(--text-primary)', fontWeight: 500, fontSize: 14 }}>{item.type}</span>
                    <span style={{ color: 'rgb(var(--inv) / 0.5)', fontSize: 13 }}>{item.count} · {fmt(item.amount)} · {item.pct?.toFixed(1)}%</span>
                  </div>
                  <div style={{ height: 6, borderRadius: 3, backgroundColor: 'rgb(var(--inv) / 0.08)' }}>
                    <div style={{ height: '100%', borderRadius: 3, background: 'linear-gradient(135deg, var(--accent), var(--accent-dark))', width: `${Math.min(100, item.pct ?? 0)}%` }} />
                  </div>
                </div>
              ))}
            </div>
            ...
            {/* Expense Breakdown */}
            <div style={{ backgroundColor: 'var(--card-bg)', borderRadius: 20, border: '1px solid rgb(var(--inv) / 0.08)', padding: 24 }}>
              <h2 style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: 16, margin: '0 0 16px' }}>Expenses by Category</h2>
              {eb.length === 0 && <p style={{ color: 'rgb(var(--inv) / 0.4)', fontSize: 14 }}>No data</p>}
              {eb.map(item => {
                const barColor = item.pctUsed > 90 ? '#f87171' : item.pctUsed > 70 ? '#f59e0b' : '#7c6bff'
                return (
                  <div key={item.category} style={{ marginBottom: 14 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                      <span style={{ color: 'var(--text-primary)', fontWeight: 500, fontSize: 14 }}>{item.category}</span>
                      <span style={{ color: 'rgb(var(--inv) / 0.5)', fontSize: 13 }}>{fmt(item.spent)} / {fmt(item.budget)} · {item.pctUsed?.toFixed(0)}%</span>
                    </div>
                    <div style={{ height: 6, borderRadius: 3, backgroundColor: 'rgb(var(--inv) / 0.08)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '200px 1fr', gap: 24, marginBottom: 24 }}>
            {/* Donut */}
            <div style={{ backgroundColor: 'var(--card-bg)', borderRadius: 20, border: '1px solid rgb(var(--inv) / 0.08)', padding: 24, display: 'flex', flexDirection: 'column' as const, alignItems: 'center' }}>
              <h2 style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: 15, margin: '0 0 16px', textAlign: 'center' }}>Gender Ratio</h2>
              <svg width="130" height="130" viewBox="0 0 130 130">
                ...
                <text x="65" y="76" textAnchor="middle" fill="rgb(var(--inv) / 0.4)" fontSize="11">Total</text>
              </svg>
              <div style={{ display: 'flex', gap: 12, marginTop: 12 }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: 5, color: 'rgb(var(--inv) / 0.6)', fontSize: 12 }}><span style={{ width: 10, height: 10, borderRadius: '50%', backgroundColor: '#a78bfa', display: 'inline-block' }} />Female {femPct.toFixed(0)}%</span>
                <span style={{ display: 'flex', alignItems: 'center', gap: 5, color: 'rgb(var(--inv) / 0.6)', fontSize: 12 }}><span style={{ width: 10, height: 10, borderRadius: '50%', backgroundColor: '#64748b', display: 'inline-block' }} />Male {malePct.toFixed(0)}%</span>
              </div>
            </div>

            {/* Categories table */}
            <div style={{ backgroundColor: 'var(--card-bg)', borderRadius: 20, border: '1px solid rgb(var(--inv) / 0.08)', padding: 24 }}>
              <h2 style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: 15, margin: '0 0 16px' }}>Membership Categories</h2>
              {[
                { label: 'Elders', value: mem.elders, color: '#f59e0b' },
                { label: 'Ministers', value: mem.ministers, color: 'var(--accent)' },
                ...
                <div key={row.label} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
                  <span style={{ color: 'rgb(var(--inv) / 0.7)', fontSize: 14, width: 80 }}>{row.label}</span>
                  <div style={{ flex: 1, height: 8, borderRadius: 4, backgroundColor: 'rgb(var(--inv) / 0.06)' }}>
                    <div style={{ height: '100%', borderRadius: 4, backgroundColor: row.color, width: `${mem.total > 0 ? Math.min(100, (row.value / mem.total) * 100) : 0}%` }} />
                  </div>
                  <span style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: 15, width: 40, textAlign: 'right' }}>{row.value}</span>
                </div>
              ))}

              {/* Quick stats footer */}
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginTop: 20, paddingTop: 16, borderTop: '1px solid rgb(var(--inv) / 0.06)' }}>
                {[
                  ...
                  <div key={s.label} style={{ textAlign: 'center' }}>
                    <p style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: 20, margin: 0 }}>{s.value}</p>
                    <p style={{ color: 'rgb(var(--inv) / 0.4)', fontSize: 11, margin: '4px 0 0' }}>{s.label}</p>
                  </div>
                ))}
              </div>
            </div>
